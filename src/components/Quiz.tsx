...
export function Quiz({words}:{words:WordEntry[]}){
  ...
  return <section className="quiz-play"><div className="quiz-progress"><span>{index+1} / {questions.length}</span><div><i style={{width:`${((index+1)/questions.length)*100}%`}}/></div></div><SimilarityBadge type={question.entry.similarityType}/><h1>{question.prompt}</h1><div className="quiz-options">{question.options.map(option=><button key={option} disabled={!!selected} className={selected?(option===question.answer?'correct':option===selected?'wrong':'muted'):''} onClick={()=>submit(option)}>{option}{selected&&option===question.answer&&<Check/>}{selected&&option===selected&&option!==question.answer&&<X/>}</button>)}</div>{selected&&<div className={`feedback ${selected===question.answer?'ok':'ng'}`}><h2>{selected===question.answer?'正解！':'もう一歩'}</h2><p><strong>{question.answer}</strong>{question.mode==='jaToZh'&&question.entry.equivalentChinese&&`　${question.entry.equivalentChinese.pinyin}`}</p><p>{question.entry.note}</p>{explanationExample&&<ExampleSentence example={explanationExample}/>}<button onClick={next}>{index+1===questions.length?'結果を見る':'次の問題'}<ArrowRight/></button></div>}</section>;
}
