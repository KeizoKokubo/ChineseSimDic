...
function Equivalent({word}:{word:WordEntry}){
  ...
  return <section className="equivalent">
    <p>🇨🇳 日本語の意味を中国語で言うなら</p>
    <div className="equivalent-word"><strong>{equivalent.simplified}</strong><SpeakButton text={equivalent.simplified} label={`${equivalent.simplified}の発音を聞く`}/>{equivalent.traditional&&equivalent.traditional!==equivalent.simplified&&<small>繁 {equivalent.traditional}</small>}<span>{equivalent.pinyin}</span><ArrowRight aria-hidden="true"/><span>{equivalent.meaning}</span></div>
...
export function WordCard({word}:{word:WordEntry}){
  ...
  return <article className="word-card">
    ...
    <div className="language-compare"><section className="ja"><h3>日本語</h3><p>{word.japanese.meaning}</p></section><section className="zh"><h3>{hasDifferentEquivalent?'⚠ 同じ漢字の中国語':'中国語'}</h3><div className="chinese-word"><strong>{word.chinese.simplified}</strong><SpeakButton text={word.chinese.simplified} label={`${word.chinese.simplified}の発音を聞く`}/>{word.chinese.traditional!==word.chinese.simplified&&<small>繁 {word.chinese.traditional}</small>}</div><em>{word.chinese.pinyin}</em><p>{word.chinese.meaning}</p></section></div>
