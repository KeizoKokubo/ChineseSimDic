...
function Equivalent({word}:{word:WordEntry}){
  ...
  return <section className="equivalent">
    <p>{word.lexicalRelation==='chineseNotStandard'?'自然な中国語では':`日本語の「${word.japanese.word}」を中国語で言うなら`}</p>
...
export function WordCard({word}:{word:WordEntry}){
  ...
  return <article className="word-card">
    ...
    <div className="language-compare"><section className="ja"><h3>日本語</h3><p>{word.japanese.meaning}</p></section><section className="zh"><h3>中国語</h3><div className="chinese-word"><strong>{word.chinese.simplified}</strong>{word.chinese.traditional!==word.chinese.simplified&&<small>繁 {word.chinese.traditional}</small>}</div><em>{word.chinese.pinyin}</em><p>{word.chinese.meaning}</p></section></div>
